...
@section('content_header')
    <h1><a href="{{ route('purchases.index') }}">Purchases</a> / Create Purchase Order</h1>
@stop
                    ...
                    <div class="form-group">
                        ...
                        <select name="supplier_id" class="form-control">
                            ...
                            @foreach($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" {{ old('supplier_id') == $supplier->id || request('supplier_id') == $supplier->id ? 'selected' : '' }}>
                                    {{ $supplier->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <tbody id="items-container">
                        <tr class="item-row">
                            <td>
                                ...
                                <input type="text" name="items[0][item_name]" class="form-control form-control-sm mt-1" placeholder="Or enter new item name">
                            </td>
                            <td><input type="number" name="items[0][quantity]" class="form-control form-control-sm qty" value="1" min="0.01" step="0.01"></td>
                            <td><input type="number" name="items[0][unit_cost]" class="form-control form-control-sm unit-cost" value="0" min="0" step="0.01"></td>
                            <td><input type="number" name="items[0][tax_rate]" class="form-control form-control-sm tax-rate" value="0" min="0" max="100" step="0.01"></td>
            ...
            <div class="row">
                <div class="col-md-6 offset-md-6">
                    <table class="table table-sm">
                        <tr><th>Subtotal:</th><td class="text-right"><span id="subtotal">0.00</span></td></tr>
                        <tr><th>Tax Total:</th><td class="text-right"><span id="tax-total">0.00</span></td></tr>
                        <tr><th>Grand Total:</th><td class="text-right"><strong><span id="grand-total">0.00</span></strong></td></tr>
                    </table>
                </div>
            </div>
...
function attachRowEvents(row) {
    row.querySelectorAll('.qty, .unit-cost, .tax-rate').forEach(el => {
        el.addEventListener('input', calculateRow);
    });
    row.querySelector('.item-select').addEventListener('change', function() {
        const opt = this.options[this.selectedIndex];
        if (opt.dataset.price) {
            row.querySelector('.unit-cost').value = opt.dataset.price;
        }
...
function calculateRow() {
    ...
    row.querySelector('.line-total').textContent = (lineTotal + taxAmount).toFixed(2);
    calculateTotals();
}
